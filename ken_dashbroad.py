...
def load_data():
    ...
    df_agg = pd.read_csv("YouTube/Aggregated_Metrics_By_Video.csv").iloc[1:, :]
    df_agg.columns = [
        "Video",
        "Video title",
        "Video publish time",
    ...
    df_agg_sub = pd.read_csv(
        "YouTube/Aggregated_Metrics_By_Country_And_Subscriber_Status.csv"
    )
    df_comments = pd.read_csv("YouTube/Aggregated_Metrics_By_Video.csv")
    df_time = pd.read_csv("YouTube/Video_Performance_Over_Time.csv")
...
if add_sidebar == "Aggregate Metrics":
    ...
    metric_medians6mo = df_agg_metrics[
        df_agg_metrics["Video publish time"] >= metric_date_6mo
    ].median()
    metric_medians12mo = df_agg_metrics[
    ...
    for i in metric_medians6mo.index:
        with columns[count]:
            delta = (metric_medians6mo[i] - metric_medians12mo[i]) / metric_medians12mo[
                i
            ]
            st.metric(
                label=i,
